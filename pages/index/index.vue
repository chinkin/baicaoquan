<template>
	<view class="container">
		...
		<view class="carousel-section">
			...
			<!-- 背景色区域 -->
			<view class="titleNview-background" :style="{backgroundColor:sTitleNViewBackground}"></view>
			<swiper class="carousel" circular @change="swiperChange">
				<swiper-item v-for="(item, index) in aCarouselList" :key="index" class="carousel-item" @click="navToProduct({title: '轮播广告'})">
					<image :src="item.src | generateUrl()" />
				</swiper-item>
			</swiper>
			<!-- 自定义swiper指示器 -->
			<view class="swiper-dots">
				<text class="num">{{iSwiperCurrent+1}}</text>
				<text class="sign">/</text>
				<text class="num">{{iSwiperLength}}</text>
			</view>
		</view>
		<!-- 分类 -->
		<view class="cate-section">
			<view @click="navToProductList(item.id)" v-for="(item, index) in aCateList" :key="index" class="cate-item">
				<image :src="item.imageurl | generateUrl()"></image>
				<text>{{item.name}}</text>
			</view>
		</view>
			...
			<swiper class="g-swiper" :duration="500">
				...
				>
					...
					<view class="g-item right">
						<image :src="aProdList[index+1].image" mode="aspectFill"></image>
						<view class="t-box">
							<text class="title clamp">{{aProdList[index+1].name}}</text>
							<view class="price-box">
								<text class="price">￥{{aProdList[index+1].baseprice}}</text> 
								<text class="m-price" v-if="aProdList[index+1].marketprice && aProdList[index+1].marketprice > 0">￥{{aProdList[index+1].marketprice}}</text> 
							</view>
							<view class="pro-box">
							  	<view class="progress-box">
							  		<progress percent="72" activeColor="#fa436a" active stroke-width="6" />
			...
			<scroll-view class="floor-list" scroll-x>
				<view class="scoll-wrapper">
					<view 
						...
						@click="navToProduct(item)"
					>
						<image :src="item.image3url | generateUrl()" mode="aspectFill"></image>
						<text class="title clamp">{{item.name}}</text>
						<text class="price">￥{{item.baseprice}}</text>
					</view>
					<view class="more">
		...
		<view class="guess-section">
			...
			>
				<view class="image-wrapper">
					<image :src="item.image2url | generateUrl()" mode="aspectFill"></image>
				</view>
				<text class="title clamp">{{item.name}}</text>
				<text class="price">￥{{item.baseprice}}</text>
			</view>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState({
				aCateList: state => {
					let aCategories = [];
					if (state.categories) {
						for (let oCategory of state.categories) {
							if (oCategory.level == 2) {
		...
		methods: {
			 ...
			 */
			async loadData() {
				let aCarouselList = await this.$api.json('carouselList');
				this.sTitleNViewBackground = aCarouselList[0].background;
				this.iSwiperLength = aCarouselList.length;
				this.aCarouselList = aCarouselList;

				//let productsList = await this.$api.json('goodsList');
			...
			swiperChange(e) {
				...
				this.iSwiperCurrent = index;
				this.sTitleNViewBackground = this.aCarouselList[index].background;
			},
			navToProductList(cId) {
				uni.navigateTo({
					url: `/pages/product/list?cId=${cId}`
				})
			},
			//商品详情页
			navToProduct(item) {
